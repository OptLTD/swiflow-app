...
function resetForm() {
  form.value = { name: '', agent: 'default', message: '', schedule: '@hourly', enabled: true }
}
...
<template>
  <div class="p-6 max-w-[960px] mx-auto">
    ...
    <div v-if="showForm" class="border border-neutral-200 rounded p-4 mb-4 bg-white space-y-2">
      ...
      <select v-model="form.agent" class="w-full border rounded px-2 py-1">
    ...
    <div class="space-y-2">
      <div v-for="j in cronStore.jobs" :key="j.id" class="border border-neutral-200 rounded p-3 bg-white">
        <template v-if="editingId === j.id">
          <select v-model="form.agent" class="w-full border rounded px-2 py-1 mb-1">
            <option v-for="a in agentsStore.agents" :key="a.key" :value="a.key">{{ a.key }}</option>
          </select>
        ...
        <template v-else>
          <div class="flex justify-between">
            <div>
              ...
              <div class="text-sm text-neutral-600">{{ j.agent }} · <span class="font-mono">{{ j.schedule }}</span></div>
